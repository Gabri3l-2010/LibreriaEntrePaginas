...
public class RegistrarSalidaInventarioController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        cmbTipoSalida.setItems(FXCollections.observableArrayList(
                ...
                MovimientoInventario.TIPO_TRASLADO,
    ...
    private void mostrarStockSeleccionado() {
        ...
        if (libro == null) {
            ...
            int stock = libroDAO.getStockDisponible(libro.getIsbn());
            libro.setStock(stock);
            lblStockActual.setText("Stock disponible: " + stock);
        }
    }
    ...
    private void handleRegistrarSalida() {
        Libro libro = cmbLibro.getValue();
        if (libro == null) {
            mostrar(Alert.AlertType.WARNING, "Libro requerido", "Selecciona el libro para registrar la salida.");
            return;
        }

        String tipo = cmbTipoSalida.getValue();
        if (tipo == null) {
            mostrar(Alert.AlertType.WARNING, "Tipo requerido", "Selecciona el tipo de salida.");
            return;
        }
        ...
        int stockActual = libroDAO.getStockDisponible(libro.getIsbn());
        if (cantidad > stockActual) {
            mostrar(Alert.AlertType.WARNING, "Stock insuficiente",
                    "No hay suficiente stock. Disponible: " + stockActual + " unidades.");
            return;
        }

        if (!SesionUsuario.getInstancia().haySesionActiva()) {
            mostrar(Alert.AlertType.ERROR, "Sesión requerida", "Inicia sesión para registrar la salida.");
            return;
        }

        String observacion = txtObservacion.getText() == null ? "" : txtObservacion.getText().trim();

        MovimientoInventario movimiento = new MovimientoInventario(
                libro.getIsbn(), tipo, cantidad,
                SesionUsuario.getInstancia().getUsuarioActual().getId(),
                observacion);

        if (movimientoDAO.registrarSalidaTransaccional(movimiento)) {
            libro.setStock(stockActual - cantidad);
            cmbLibro.getSelectionModel().select(libro);
            mostrarStockSeleccionado();
            txtCantidad.clear();
            txtObservacion.clear();
            mostrar(Alert.AlertType.INFORMATION, "Salida registrada",
                    "Se descontaron " + cantidad + " unidades y se registró el movimiento de tipo " + tipo + ".");
        } else {
            mostrar(Alert.AlertType.ERROR, "No se pudo registrar",
                    "No se descontó el stock. Verifica los datos e intenta nuevamente.");
        }
    }
    ...
    private void handleLimpiar() {
        ...
        cmbTipoSalida.getSelectionModel().clearSelection();
        txtCantidad.clear();
        txtObservacion.clear();
        lblStockActual.setText("Stock disponible: —");
    }
